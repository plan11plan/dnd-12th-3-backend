...
public class GetMostCommentedIncidentsWithinScreenUseCase {
	...
	public IncidentCursorResponse execute(
		...
		List<IncidentInfoDto> sortedIncidents = incidentCursor.contents().stream()
			.sorted(Comparator.comparingInt(IncidentInfoDto::getCommentCount).reversed())
			.collect(Collectors.toList());

		List<IncidentDistanceDto> distanceDtos = sortedIncidents.stream()
			.map(incident -> {
